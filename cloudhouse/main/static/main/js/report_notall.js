$(document).on('click', '#report', function() {
  ...
  var end_date = $('#to').val();
  var energy = $('#energyrep').is(":checked");
  var temp = $('#temprep').is(":checked");
  var voice = $('#voicerep').is(":checked");
...
if (!$('#energyrep').is(':checked') && !$('#voicerep').is(':checked') && !$('#temprep').is(':checked')) {
  $('#emsudr').show();
  $('#emsudr').text('Выберите хотя бы один датчик');
  setTimeout(function() {
    $('#emsudr').hide();
  }, 3000);
  return;
  ...
  $.ajax({
    url: '/report_sens/',
    method: 'POST',
    data: {'csrfmiddlewaretoken': csrfToken, 'start_date': start_date, 'end_date': end_date, 'temprep': temp, 'energyrep': energy, 'voicerep': voice},
